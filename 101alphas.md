# WorldQuant 101 Formulaic Alphas 表达式整理

来源：
- 论文：`101 Formulaic Alphas`
...
- 以下基本按论文附录 A 的原始写法整理。
- `? :` 表示三元条件表达式。
- `advN` 表示过去 `N` 日的平均成交额/平均成交量指标；论文原文使用 `Average Daily Dollar Volume` 系列表达输入。
- 文中常见字段：`open, high, low, close, volume, vwap, returns`。

## 常用算子速记

- `rank(x)`: 截面排序
- `ts_rank(x, N)`: 时间序列窗口内排序
- `delay(x, N)`: 滞后 `N` 期
- `delta(x, N)`: `x - delay(x, N)`
- `sum(x, N)`: 过去 `N` 期求和
- `product(x, N)`: 过去 `N` 期连乘
- `stddev(x, N)`: 过去 `N` 期标准差
- `correlation(x, y, N)`: 过去 `N` 期相关系数
- `covariance(x, y, N)`: 过去 `N` 期协方差
- `signedpower(x, a)`: 保留符号幂
- `scale(x)`: 按绝对值总和缩放
- `ts_min(x, N)`: 窗口最小值
- `ts_max(x, N)`: 窗口最大值
- `ts_argmin(x, N)`: 窗口最小值位置
- `ts_argmax(x, N)`: 窗口最大值位置
- `decay_linear(x, N)`: 线性衰减加权
- `indneutralize(x, g)`: 按行业分组中性化
...
Alpha#21:
((((sum(close, 8) / 8) + stddev(close, 8)) < (sum(close, 2) / 2)) ? (-1 * 1) : ((((sum(close, 2) / 2) < ((sum(close, 8) / 8) - stddev(close, 8))) ? 1 : (((1 < (volume / adv20)) ? 1 : (-1 * 1))))))
...
Alpha#34:
rank((((1 - rank((stddev(returns, 2) / stddev(returns, 5)))) + (1 - rank(delta(close, 1)))) + (1 - rank(ts_rank((volume / adv20), 1)))))
...
Alpha#46:
((0.25 < (((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) / 10))) ? (-1 * 1) : (((( ((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) / 10)) < 0)) ? 1 : (-1 * (close - delay(close, 1)))))
...
Alpha#49:
((((((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) / 10)) < (-1 * 0.1)) ? 1 : (-1 * (close - delay(close, 1)))))

Alpha#50:
(-1 * ts_max(rank(correlation(rank(volume), rank(vwap), 5)), 5))

Alpha#51:
((((((delay(close, 20) - delay(close, 10)) / 10) - ((delay(close, 10) - close) / 10)) < (-1 * 0.05)) ? 1 : (-1 * (close - delay(close, 1)))))
...
Alpha#64:
((rank(correlation(sum(((open * 0.178404) + (low * (1 - 0.178404))), 13.6971), sum(adv120, 13.6971), 8.62571)) < rank(delta(((((high + low) / 2) * 0.178404) + (vwap * (1 - 0.178404))), 1.46063))) * -1)
...
Alpha#75:
((rank(correlation(vwap, volume, 4.24304)) < rank(correlation(rank(low), rank(adv50), 12.4413))) * -1)
...
Alpha#79:
((rank(delta(indneutralize(((close * 0.60733) + (open * (1 - 0.60733))), IndClass.sector), 1.23438)) < rank(correlation(ts_rank(vwap, 3.60973), ts_rank(adv150, 9.18637), 14.6644))) * -1)
...
Alpha#85:
((rank(correlation(((high * 0.876703) + (close * (1 - 0.876703))), adv30, 9.61331))^rank(correlation(ts_rank(((high + low) / 2), 3.70596), ts_rank(volume, 10.1595), 7.11408))) * -1)
...
Alpha#88:
(min(rank(decay_linear(((rank(open) + rank(low)) - (rank(high) + rank(close))), 8.06882)), ts_rank(decay_linear(correlation(ts_rank(close, 8.44728), ts_rank(adv60, 20.6966), 8.01266), 6.65053), 2.61957)) * -1)
...
Alpha#96:
-1 * (max(ts_rank(decay_linear(correlation(rank(vwap), rank(volume), 3.83878), 4.16783), 8.38151), ts_rank(decay_linear(ts_argmax(correlation(ts_rank(close, 7.45404), ts_rank(adv60, 4.13242), 3.65459), 12.6556), 14.0365), 13.4143)))
...
Alpha#99:
((rank(correlation(sum(((high + low) / 2), 20.8187), sum(adv60, 20.8187), 8.62571)) < rank(correlation(low, volume, 6.28259))) * -1)
...
- 论文中部分因子直接含有 `IndClass.sector / industry / subindustry` 中性化操作。
- 部分窗口长度是浮点数；实做时通常会按具体框架取整或按论文作者代码约定处理。
- `Alpha#56` 中使用了 `cap`，即市值。
